...
const leaks=[];
// 9/26: 검사 대상을 배포되는 모든 글자 파일로 넓힘(expr.js 에 이름이 남아 있던 것을 놓쳤음)
const walk=d=>fs.readdirSync(d,{withFileTypes:true}).flatMap(e=>e.name.startsWith('.')||e.name==='node_modules'?[]:e.isDirectory()?walk(d+'/'+e.name):[d+'/'+e.name]);
const targets=walk('.').map(f=>f.slice(2)).filter(f=>/\.(html|js|css|json|webmanifest|md|svg)$/.test(f)&&!['CLAUDE.md','_check.cjs'].includes(f));
